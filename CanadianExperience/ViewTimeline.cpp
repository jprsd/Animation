...
BEGIN_MESSAGE_MAP(CViewTimeline, CScrollView)
    ...
    ON_WM_TIMER()
	ON_COMMAND(ID_EDIT_TREE1SEED32792, &CViewTimeline::OnEditTree1seed32792)
	ON_COMMAND(ID_EDIT_TREE2SEED, &CViewTimeline::OnEditTree2seed)
	ON_COMMAND(ID_EDIT_SETTREE1GROWFRAME, &CViewTimeline::OnEditSettree1growframe)
	ON_COMMAND(ID_EDIT_SETTREE1HARVESTFRAME, &CViewTimeline::OnEditSettree1harvestframe)
	ON_COMMAND(ID_EDIT_SETTREE2GROWFRAME32797, &CViewTimeline::OnEditSettree2growframe32797)
	ON_COMMAND(ID_EDIT_SETTREE2HARVESTFRAME, &CViewTimeline::OnEditSettree2harvestframe)
END_MESSAGE_MAP()
...
void CViewTimeline::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    {
        double time = (double)((double)point.x - (double)BorderLeft) / ((double)timeline->GetFrameRate() * (double)TickSpacing);
        if (time < 0)
        {
            time = 0;
        }
...
/**
 * Handles the tree seed dialog box for the first tree drawable
 */
 void CViewTimeline::OnEditTree1seed32792()
 {
	 mTreeDrawable1->ShowDialogBox();
 }

/**
 * Handles the tree seed dialog box for the second tree drawable
 */
 void CViewTimeline::OnEditTree2seed()
 {
	 mTreeDrawable2->ShowDialogBox();
 }

/**
 * Set the start frame for the first tree drawable (i.e. "plant" the tree)
 */
 void CViewTimeline::OnEditSettree1growframe()
 {
	 mTreeDrawable1->SetStartFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
 }

/**
 * Set the harvest frame for the first tree drawable
 */
 void CViewTimeline::OnEditSettree1harvestframe()
 {
	 mTreeDrawable1->SetHarvestFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
 }

/**
 * Set the start frame for the second tree drawable (i.e. "plant" the tree)
 */
 void CViewTimeline::OnEditSettree2growframe32797()
 {
	 mTreeDrawable2->SetStartFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
 }

/**
 * Set the harvest frame for the second tree drawable
 */
 void CViewTimeline::OnEditSettree2harvestframe()
 {
	 mTreeDrawable2->SetHarvestFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
 }
